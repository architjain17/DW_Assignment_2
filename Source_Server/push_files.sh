...
{		
	if [ ! -d ${BASE_PATH}/logs_shipping ]; then
		mkdir ${BASE_PATH}/logs_shipping 2>&1 | tee -a log.txt
		func_check_error ${PIPESTATUS[0]} "Error in creating directory logs_shipping. Please check!"
	fi

	if [ ! -d ${BASE_PATH}/logs_process ]; then
		mkdir ${BASE_PATH}/logs_process 2>&1 | tee -a log.txt
		func_check_error ${PIPESTATUS[0]} "Error in creating directory logs_process. Please check!"
	fi
	...
	if [ ! -d ${BASE_PATH}/logs_failed ]; then
		mkdir ${BASE_PATH}/logs_failed 2>&1 | tee -a log.txt
		func_check_error ${PIPESTATUS[0]} "Error in creating directory logs_failed. Please check!"
	fi 
}
...
func_cleanup_on_skip ()
{
	...
	for file in `ls ${BASE_PATH}/logs_process/$tar_name.split*`;
	do
	...
	mv ${BASE_PATH}/logs_process/$tar_name.split* ${BASE_PATH}/logs_failed/ 2>&1 | tee -a log.txt
	func_check_error ${PIPESTATUS[0]} "Error in moving files from logs_process folder to logs_failed folder. Please check!"
...
func_push ()
{
	...
	filename=`basename $file`
	scp ${BASE_PATH}/logs_process/$filename source@$intermediate_server_ip:${SCP_DEST_PATH} >/dev/null 2>&1 | tee -a log.txt
	...
	exit_status=`echo ${PIPESTATUS[0]}`
		...
		if [ $exit_status -eq 0 ]; then
			...
			mv ${BASE_PATH}/logs_process/$filename ${BASE_PATH}/logs_backup/$filename >/dev/null 2>&1 | tee -a log.txt
			...
			if [ $exit_status -eq 67 ]; then
				...
				# checking internet connectivity with intermediate server to decide whether to continue or not
				# uncomment next line and pass address of intermediate server to check connectivity
				ping -q -c 1 $intermediate_server_ip >/dev/null 2>&1 | tee -a log.txt
			...
			mv ${BASE_PATH}/logs_process/$filename ${BASE_PATH}/logs_failed/$filename >/dev/null 2>&1 | tee -a log.txt
...
func_push_failed_files()
{
	...
	for file in `ls ${BASE_PATH}/logs_failed/*.split* 2>&1 | tee -a log.txt`;
	do
	...
	for file in `ls ${BASE_PATH}/logs_failed/*.txt > /dev/null 2>&1 | tee -a log.txt`;
	do
		exit_status=`func_push $file`
...
func_push_rotated_log_files()
{
	...
	echo "md5sum=" >> log.txt
	md5sum=`md5sum ${BASE_PATH}/logs_process/$tar_name | awk '{ print $1 }' 2>&1 | tee -a log.txt`
	echo "md5sum="$md5sum  > ${BASE_PATH}/logs_process/$tar_name.txt

	# spliting file into smaller files of size 1 GB each in order to ship
	split -b $size_of_one_part -d ${BASE_PATH}/logs_process/$tar_name ${BASE_PATH}/logs_process/$tar_name.split 2>&1 | tee -a log.txt
	...
	if [ $split_exit_status -eq 0 ]; then
		rm ${BASE_PATH}/logs_process/$tar_name 2>&1 | tee -a log.txt
		func_check_error ${PIPESTATUS[0]} "Error in removing files from logs_process folder. Please check!"
	fi

	total_parts=`ls ${BASE_PATH}/logs_process/$tar_name.split* | wc -l`
	...
	echo "total_parts="$total_parts >> ${BASE_PATH}/logs_process/$tar_name.txt
	scp ${BASE_PATH}/logs_process/$tar_name.txt source@$intermediate_server_ip:${SCP_DEST_PATH} 2>&1 | tee -a log.txt
	...
	if [ $scp_exit_status -eq 0 ]; then 
		mv ${BASE_PATH}/logs_process/$tar_name.txt ${BASE_PATH}/logs_backup/ 2>&1 | tee -a log.txt
		func_check_error ${PIPESTATUS[0]} "Error in moving files from logs_process folder to logs_backup. Please check!"
	else
		mv ${BASE_PATH}/logs_process/$tar_name.txt ${BASE_PATH}/logs_failed/ 2>&1 | tee -a log.txt
		func_check_error ${PIPESTATUS[0]} "Error in moving files from logs_process folder to logs_failed. Please check!"
	fi
}
...
# if destination folder is not present, creating it
# ssh source@$intermediate_server_ip mkdir -p /home/source/log > /dev/null 2>&1
# exit_status=$?

# if [ $exit_status = 67 ]; then
# 	bash mail_notification.sh alert.template "SSH key verfication failed"

# elif [ $exit_status = 255 ]; then
# 	bash mail_notification.sh alert.template "Intermediate server with IP $intermediate_server_ip is not available"
# fi


# check if failed files present in folder logs_failed
if [ `ls ${BASE_PATH}/logs_failed/*.split* 2> /dev/null | head -1` ]; then
	# retry sending failed files
	func_push_failed_files
fi

# check if new log files present in folder logs_failed
if [ `ls ${BASE_PATH}/logs/raw_*_${servername}_db.log.rotated.*.log 2> /dev/null | head -1` ]; then

	# moving all log files to logs_shipping folder

	mv ${BASE_PATH}/logs/raw_*_${servername}_db.log.rotated.*.log ${BASE_PATH}/logs_shipping/ 2>&1 | tee -a log.txt
	func_check_error ${PIPESTATUS[0]} "Error in moving files from logs folder to logs_shipping. Please check!"

	# making an archive and moving it to logs_shipping folder

	tar_name=${servername}`date +"%F%H%M"`.tar.gz
	
	# removing log files if tar is successfully created

	# make an archieve of all log files
	...
	func_check_error $tar_exit_status "Error in creating tar file from log files present in folder logs_shipping. Please check!"

	# if tar is successfully created then remove log files
	if [ $tar_exit_status -eq 0 ]; then
		rm ${BASE_PATH}/logs_shipping/raw_*_${servername}_db.log.rotated.*.log 2>&1 | tee -a log.txt
		func_check_error ${PIPESTATUS[0]} "Error in deleting files from logs_shipping folder. Please check!"
	fi

	mv ${BASE_PATH}/logs_shipping/$tar_name ${BASE_PATH}/logs_process/ 2>&1 | tee -a log.txt
	func_check_error ${PIPESTATUS[0]} "Error in moving files from logs_shipping folder to logs_process. Please check!"

	# sending new rotated log files
	func_push_rotated_log_files $tar_name
...
if [ -f run_details_ship_success.txt ] || [ -f run_details_ship_failure.txt ]; then
	...
	bcp [${local_sql_server_database_name}].shipping.log in "database.log" -S $local_sql_server_ip_address -U $local_sql_server_username -P $local_sql_server_password -f database_log.fmt 2>&1 | tee -a log.txt
